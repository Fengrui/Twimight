...
public class HtmlService extends Service {
	...
	private void bulkDownloadTweetHtmlPages(boolean forced){
		...
		if((System.currentTimeMillis() - lastTime) < 1000*30){
			...
			Cursor allHtml = htmlDbHelper.getAll();
			...
			long fileLength = 0;
			long maxLength = 0;
			long minLength = 999999999;
			for (allHtml.moveToLast(); !allHtml.isBeforeFirst(); allHtml.moveToPrevious())
			{
				String userId1 = allHtml.getString(allHtml.getColumnIndex(HtmlPage.COL_USER));
				String filename1 = allHtml.getString(allHtml.getColumnIndex(HtmlPage.COL_FILENAME));
				if(allHtml.getInt(allHtml.getColumnIndex(HtmlPage.COL_DOWNLOADED)) == 1){
					alldownloaded ++;
					totalTries = totalTries + allHtml.getInt(allHtml.getColumnIndex(HtmlPage.COL_TRIES));
					String[] filePath = {HtmlPage.HTML_PATH + "/" + userId1};
					if(sdCardHelper.checkSDStuff(filePath)){
						File targetFile = sdCardHelper.getFileFromSDCard(filePath[0], filename1);
						fileLength = fileLength + targetFile.length();
						maxLength = Math.max(targetFile.length(), maxLength);
						minLength = Math.min(targetFile.length(), minLength);
					}
					
				}else if(allHtml.getInt(allHtml.getColumnIndex(HtmlPage.COL_TRIES)) == 15){
					failedNo ++;
				}
			}
			
			Log.d("evaluation", "fail rate=" + String.valueOf((float) failedNo/ (float) (alldownloaded + failedNo)));
			Log.d("evaluation", "average tries="+ String.valueOf((float) totalTries / (float) (alldownloaded + failedNo)));
			Log.d("evaluation", "average file size=" + String.valueOf((float) fileLength / (float) alldownloaded));
			Log.d("evaluation", "max file size=" + String.valueOf(maxLength));
			Log.d("evaluation", "min file size=" + String.valueOf(minLength));
			Log.d("evaluation", "total file no=" + String.valueOf((alldownloaded + failedNo)));
